...
export function getCardsPerPlayer(playerCount: number): number {
  return GAME_CONSTANTS.CARDS_PER_PLAYER[playerCount as keyof typeof GAME_CONSTANTS.CARDS_PER_PLAYER] || 0;
}
...
export function createTrio(cards: Card[]): Trio | null {
  ...
  return {
    ...
    cards: cards as [Card, Card, Card],
    formedAt: Date.now(),
  };
}

// Vérifier les conditions de victoire
export function checkVictoryConditions(player: Player): VictoryResult {
...
export function getSmallestCard(hand: Card[]): Card | null {
...
export function getLargestCard(hand: Card[]): Card | null {
  if (hand.length === 0) return null;
...
export function hasTwoDifferentNumbers(numbers: number[]): boolean {
...
export function hasTwoConsecutiveDifferentNumbers(numbers: number[]): boolean {
...
export function extractNumbers(cards: Card[]): number[] {
  return cards.map(card => card.number);
}

// Vérifier si tous les numéros sont identiques
export function allSameNumber(numbers: number[]): boolean {
...
export function getNextPlayer(gameState: GameState): Player | null {
...
export function calculateAIDelay(difficulty: import('../types/core.js').AIDifficulty): number {
...
export function isPlayerConnected(player: Player): boolean {
  return player.connectionStatus === ConnectionStatus.CONNECTED;
}

// Obtenir les joueurs connectés
export function getConnectedPlayers(players: Player[]): Player[] {
...
export function canRevealCard(card: Card): boolean {
  return !card.isRevealed;
}

// Obtenir les cartes disponibles pour révélation au centre
export function getRevealableCenterCards(centerCards: Card[]): Card[] {
