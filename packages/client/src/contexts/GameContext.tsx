...
export const GameProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Ref pour éviter les doubles connexions
  ...
  const connectSocket = () => {
    ...
    newSocket.on('roomUpdated', (roomState) => setRoomState(roomState))
    ...
    newSocket.on('trioFailed', (playerId) => {
      console.log('Échec trio:', playerId)
    })
  ...
  // Nettoyage uniquement au démontage du Provider
  useEffect(() => {
    return () => {
      if (socket) socket.disconnect()
    }
  }, []) // ← tableau vide, pas [socket]
  ...
  return (
    <GameContext.Provider value={{
      ...
      state: {
        playerInfo, currentView, isLoading, error,
        isConnected, playerId, roomState,
        gameState, chatMessages: roomState?.chatMessages || [],
