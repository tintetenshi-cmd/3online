...
export class AIEngine {
  ...
  private selectAction(actionScores: ActionScore[], strategy: AIStrategy): GameAction {
    ...
    const difficulty = (strategy as any).difficulty || 'MEDIUM';
    switch (difficulty) {
   ...
   */
  private estimateTrioCompletionChance(action: GameAction, targetNumber: number, analysis: GameAnalysis): number {
   ...
   */
  private estimateInformationGain(action: GameAction, gameState: GameState, analysis: GameAnalysis): number {
   ...
   */
  private assessActionRisk(action: GameAction, gameState: GameState, aiPlayer: AIPlayer, analysis: GameAnalysis): number {
   ...
   */
  private estimateOpponentDisruption(action: GameAction, gameState: GameState, analysis: GameAnalysis): number {
  ...
  private calculateTurnsSinceLastTrio(gameState: GameState): number {
    ...
    for (let i = gameState.turnHistory.length - 1; i >= 0; i--) {
      ...
      if (turns > 10) break; // Limite arbitraire
    }
    return turns;
  }
